...
class XORNeuralNetwork:
    def __init__(self, seed: int = 42, learning_rate: float = 0.01):
        ...
        self.mean_squared_error: Callable[[float, float], float] = lambda output, answer: 1/2 * (output - answer) ** 2
        self.mean_squared_error_derivative: Callable[[float, float], float] = lambda output, answer: output - answer
    ...
    def backward(self, input: Input, answer: Output) -> float:
        ...
        gradient_loss = self.mean_squared_error_derivative(self.hidden_to_output_outputs[0], answer.y)
        gradient_output_local = self.sigmoid_derivative(self.hidden_to_output_outputs[0])
        ...
        gradient_n1 = gradient_output * self.hidden_to_output_weights[0] * gradient_n1_local
        gradient_n2 = gradient_output * self.hidden_to_output_weights[1] * gradient_n2_local
